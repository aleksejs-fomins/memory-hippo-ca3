...
class SinglePopulationDiscrete:
        ...
        for rowPopKey in self.p['POP_KEYS']:
            ...
            if p['WITH_SYNAPSES']:
                for colPopKey in self.p['POP_KEYS']:
                    ...
                    WRangeRow = p['W_RANGE_' + rowPopKey]
                    self.v[WKey] = aux.spRandUMat((N_COL, N_ROW), WRangeRow, p['P_CONN'][rckey])
    ...
    def nextState(self, rowPopKey, VI, param_sim, vnew):
        ...
        if self.p['WITH_SYNAPSES']:
            for colPopKey in self.p['POP_KEYS']:
                ...
                W_THIS = self.v['W_' + colPopKey + '_' + rowPopKey]
                V_PRE += W_THIS.dot(V_COL)
    ...
    def nextWeights(self, rowPopKey, colPopKey, vnew):
        ...
        if SP_RATE_THIS != 0:
            ...
            T4= time() - t
            print('timing', round(T1, 2), round(T2, 2), round(T3, 2), round(T4, 2))
        else:
            vnew[WKey] = WThis
